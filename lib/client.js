...
const fetchStream = require('./fetchStreamReadable.js');
...
class Client {
  ...
  /**
   * Make a streaming query call to the API using fetch api.
   *
   * @param {Object} options Configuration values.
   * @param {Object} callbacks An object with attributes for callbacks:
   *  - meta: Function to send headers.
  ...
  streamFetch(options, callbacks) {
    ...
    return fetchStream.create(options).stream(opc, callbacks)
  }
